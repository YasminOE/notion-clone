...
export const Navigation = () => {
  ...
  // Dynamic sidebar when screen changes from desktop to mobile
  useEffect(() => {
    if (isMobile) {
      collapse();
    } else {
  ...
  // handle page pathname, when sidebar expands on mobile screens
  useEffect(() => {
    if (isMobile) {
      collapse();
    } else {
  ...
  const resetWidth = () => {
    if (sidebarRef.current && navbarRef.current) {
      ...
      //this is for duration animation
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-4">
          <DocumentList />
          <Item label="New Page" icon={Plus} onClick={handleCreate} />
          <Popover>
          ...
          <nav className="bg-transparent px-3 py-2 w-full">
            {isCollapsed && (
              <MenuIcon
                onClick={resetWidth}
                role="button"
                className="w-6 h-6 t ext-muted-foreground"
              />
            )}
          </nav>
        )}
